...
def get_parsing_rules(metrics : list, dimensions :list, month_first, month_last, title_report : bool, title_identifiers : list, platform_name: str):
    prompt=f"""
...
You know from the user that metrics in this file are: {metrics}"""
    prompt += f""" .That means that you should try to locale cells with these metrics in them. 
The date should span months from {month_first} to {month_last}."""
    if len(dimensions) > 0:
        prompt += f""" The user also told you that the dimensions in this file are: {dimensions}.
        That means that you should try to locale cells with these dimensions in them."""
    else:
        prompt += """ There are no dimensions in this file."""
    
    if title_report:
        prompt += f""" The user also told you that this is a title report, which means you should include information about title and title identifiers.
        The title identifiers are {title_identifiers} """
    else:
        prompt += """ The user also told you that this is not a title report, which means you should not include information about title and title identifiers"""

    prompt += f"""You alse know this data is from platform {platform_name}."""
...
After you finish creating the parsing rules, give them to check_parsing_rules function as string, this function will validate the format, and either return True, which means you did it correctly, or 
it will return validation errors. If it returns the errors, you should fix them and try again, until it passes.
If it passes, return only the parsing rules json in the final response, without any comments, so that it will pass final validation.
...
#All source definitions (dates, metrics) require an extract_params block, even if empty ({}).